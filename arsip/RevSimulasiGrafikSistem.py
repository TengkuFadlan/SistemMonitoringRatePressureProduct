...
# Threshold klasifikasi berbasis baseline rest
LOW_MULT = 1.20
MED_MULT = 1.50
HIGH_MULT = 1.80
...
# Penyimpanan analitik lintas sesi
all_window_logs = []
current_session_logs = []

# Penyimpanan baseline & histori
rest_rpp_values = []
post_rpp_values = []
recovery_rpp_values = []
...
def classify_load(rpp, baseline_rpp, phase):
    if baseline_rpp is None:
        return "Mengumpulkan baseline", "#aaaaaa"
...
def classify_recovery(rpp, baseline_rpp, post_peak_rpp):
    if baseline_rpp is None or post_peak_rpp is None:
        return "Recovery belum dapat dinilai", "#aaaaaa", None, None
...
def reset_session_display_only():  # Ganti namanya agar lebih jelas
    ...
    current_session_logs = []
    # JANGAN reset baseline_rpp, rest_rpp_values, atau post_peak_rpp di sini!
...
def start_phase_session(phase_name):
    global app_mode, selected_phase, session_id, session_start_time
    session_id += 1
    selected_phase = phase_name
    session_start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    reset_session_display_only()  # Gunakan fungsi yang tidak menghapus baseline
...
menu_info = ax_menu.text(
    0.5,
    0.20,
    "Saat monitoring: tekan [M] untuk kembali ke menu dan simpan sesi, tekan [Q] untuk keluar",
    fontsize=13,
    color="#aaaaaa",
    ha="center",
    va="center",
...
def update(frame):
    ...
    try:
        ...
        if sample_count >= BUF_SZ and sim_idx % 60 == 0:
            ...
            if delta_rpp is not None:
                delta_text.set_text(f"ΔRPP: {delta_rpp:+.0f}")
            else:
                delta_text.set_text("ΔRPP: --")

            if rec_pct is not None:
                recovery_pct_text.set_text(f"%Recovery: {rec_pct:.1f}%")
            else:
                recovery_pct_text.set_text("%Recovery: --")
            ...
            # refresh tampilan walau belum ada window valid
            if last_sbp_val is not None:
                sbp_text.set_text(f"SBP: {last_sbp_val:.1f} mmHg")
            else:
                sbp_text.set_text("SBP: --")

            if last_hr_val is not None:
                hr_text.set_text(f"HR: {last_hr_val:.1f} BPM")
            else:
                hr_text.set_text("HR: --")

            if last_rpp_val is not None:
                rpp_text.set_text(f"Current RPP: {last_rpp_val:.0f}")
            else:
                rpp_text.set_text("Current RPP: --")

            phase_text.set_text(f"Phase: {selected_phase} | Session: {session_id}")
            load_status_text.set_text(f"Status Beban: {last_load_label}")
            load_status_text.set_color(last_load_color)
            recovery_status_text.set_text(f"Status Recovery: {last_recovery_label}")
            recovery_status_text.set_color(last_recovery_color)

            if last_delta_rpp is not None:
                delta_text.set_text(f"ΔRPP: {last_delta_rpp:+.0f}")
            else:
                delta_text.set_text("ΔRPP: --")

            if last_recovery_pct is not None:
                recovery_pct_text.set_text(f"%Recovery: {last_recovery_pct:.1f}%")
            else:
                recovery_pct_text.set_text("%Recovery: --")
